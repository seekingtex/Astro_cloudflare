---
publishDate: 2025-01-25
title: Foida AirDeck 270
sku: FD-AIR-270
summary: Lightweight 2.7 m inflatable dinghy with a drop-stitch air deck. Stows in a car boot.
description: A 2.7 m inflatable with a high-pressure drop-stitch air deck. Packs down to a bag that fits in a small car boot, inflates in 10 minutes, planes with a 6 hp outboard.
image: https://images.unsplash.com/photo-1605281317010-fe5ffe798166?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80
gallery:
  - url: https://images.unsplash.com/photo-1605281317010-fe5ffe798166?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80
    alt: Foida AirDeck 270 — inflated
  - url: https://images.unsplash.com/photo-1525909002-1b05e0c869d8?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80
    alt: Foida AirDeck 270 — on the water
category: inflatable
tags:
  - inflatable
  - tender
  - portable
specs:
  - label: Overall length
    value: 2.70 m
  - label: Beam
    value: 1.50 m
  - label: Tube material
    value: 1.0 mm PVC
  - label: Deck
    value: Drop-stitch high-pressure air deck
  - label: Max load
    value: 350 kg
  - label: Max persons
    value: 3
  - label: Recommended power
    value: 6 hp
  - label: Weight (boat only)
    value: 28 kg
  - label: Chambers
    value: 3
  - label: Bag size
    value: 110 × 60 × 35 cm
price:
  amount: '1,650'
  currency: USD
  note: Includes bag, pump, oars and repair kit
inStock: true
featured: true
metadata:
  title: Foida AirDeck 270 — 2.7 m portable inflatable
  description: The Foida AirDeck 270 is a 2.7 m inflatable with a drop-stitch air deck. 28 kg, packs to a car-boot bag, planes with a 6 hp outboard.
---

## A dinghy that disappears in a bag

The AirDeck 270 is for owners who need a tender that lives in a locker. It inflates in about ten minutes with the included pump, planes confidently with a 6 hp outboard, and packs back into a bag small enough for a hatchback.

### Hull and deck

- 1.0 mm PVC tube with 3 chambers
- High-pressure drop-stitch air deck — rigid under foot, soft when deflated
- Self-bailing when under way

### What's in the bag

- Boat, bag, pump, oars, repair kit
- 25 L fuel tank as optional extra

### Best for

- Yacht tenders (sub-10 m)
- Weekenders with a small car
- Anyone who has ever wrestled a heavy dinghy onto a beach
